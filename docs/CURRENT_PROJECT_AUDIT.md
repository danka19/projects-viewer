...
- Human presentation decision: use a layered dependency graph for `Specs` view. Independent specifications occupy graph lanes without connecting lines; explicit prerequisite edges run from prerequisite to dependent work. The graph must be laid out deterministically from validated dependencies rather than file order, and status remains a property of each node rather than the primary grouping axis.
- Status: source-boundary, spec/task hierarchy, dependency semantics, primary-view selection, and the layered-graph direction are accepted; detailed node interaction, dense/mobile fallback, and error behavior remain in design discovery. No implementation is authorized until the design is approved.
