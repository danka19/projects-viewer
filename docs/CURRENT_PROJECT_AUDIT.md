...
| [`improve-dashboard-evidence-trust`](../openspec/changes/improve-dashboard-evidence-trust/) | `implemented_verified` | 11/13 tasks are checked. Scanner/search behavior, focused and full automated gates, a fresh configured-project rescan, and the corrected desktop/mobile browser matrix pass. Documentation/task-state and final OpenSpec CLI follow-through remain; the change is intentionally active, unsynced, and unarchived. |
...
Sequential work: timeline archival/integration is blocked only by explicit human acceptance task 7.6. MCP/API hardening is complete and archived independently of timeline geometry. Dashboard evidence-trust implementation and acceptance evidence are complete as the follow-on hardening slice, but its documentation/task-state and strict OpenSpec CLI follow-through remain before any separate archive action. Selectable Specs Canvas human product acceptance remains independent and open.
...
| Dashboard evidence-trust remediation | Active change `improve-dashboard-evidence-trust` excludes completed checkbox blockers, cross-line OpenSpec normative blocker text, and non-action next-action prose; search exposes query-centered fragments and deduplicates identical file/line/text evidence across result representations. Focused scanner/search verification, 133/133 Node plus 104/104 Vitest full gates, production build, a fresh 83-file configured-project rescan, and corrected 1280x720/390x844 browser acceptance pass. The dated API/UX trust audit owns exact commands, root causes, counts, and residual limitations. |
...
| AUDIT-017 | [`API_UX_TRUST_AUDIT_2026-07-12.md`](audits/API_UX_TRUST_AUDIT_2026-07-12.md) records four remediated and verified trust findings. Archived `harden-mcp-context-api` solely owns the completed API fallback remediation. Active [`improve-dashboard-evidence-trust`](../openspec/changes/improve-dashboard-evidence-trust/) owns the implemented and verified scanner/search remediation; documentation/task-state and strict OpenSpec CLI follow-through remain before any explicit sync/archive step. Timeline clarity and selectable Specs Canvas human acceptance remain separate open gates. | Phase 4 hardening changes | remediation_verified; change_open 2026-07-12 |
