...
- Status: source-boundary, spec/task hierarchy, dependency semantics, and primary-view selection are accepted; the Trello-plus-mindboard composition, detailed node interaction, dense/mobile fallback, and error behavior remain in design discovery. No implementation is authorized until the design is approved.
