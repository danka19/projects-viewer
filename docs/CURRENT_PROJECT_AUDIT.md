...
- Status: source-boundary, spec/task hierarchy, dependency semantics, and per-project view persistence are accepted; the first-visit tie-breaker without configuration, dependency-graph presentation, and fallback details remain in design discovery. No implementation is authorized until the design is approved.
