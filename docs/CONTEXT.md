...
| Agent preflight packet | Accepted AI-agent-oriented JSON packet for required reading, active change context, acceptance mapping, attention signals, verification expectations, and safety boundaries before agent work. | Canonical behavior lives in `openspec/specs/agent-preflight-packet/`; implementation history is archived under `openspec/changes/archive/2026-07-15-agent-preflight-packet/`. It is separate from the daily/weekly human project brief and does not start agent work. |
...
| Primary work view | The per-project selected `Roadmap` or `Specs` visualization. | Selection is presentation state persisted by stable project id; it never changes lifecycle truth or the scanned project. |
| Documentation view roots | Optional saved project-relative roots that scope Roadmap or Specs extraction. | Roots enter scanning only through validated canonical config; empty roots mean safe mixed-repository classification. |
| Specs Canvas | Responsive read-only Canvas Focus representation of specification cards, owned tasks, and explicit dependencies. | Desktop/tablet uses a locally pannable routed graph; mobile uses dependency-aware vertical groups. |
| Explicit specification dependency | A prerequisite relationship declared by supported `work.dependsOn` identity frontmatter. | Direction is prerequisite to dependent; file order, names, dates, numbering, lifecycle, heuristics, and model judgment never create an edge. |
| Unassigned work | A task with no supported ownership evidence. | It stays in a separate region and never becomes a fabricated specification. |
...
- Dashboard selection, primary-view/canvas state, timeline expansion, and detail-drawer state are presentation state only; they must not alter lifecycle status or accepted project decisions.
