...
- Final review exposed a second representation mismatch from one scanner source checkbox: `runScan()` emitted `- [ ] Release is blocked by the missing signing key.` as blocker text retaining `[ ]` and as task text without the marker. The blocker still used its kind-specific stable key while the task used shared evidence identity, so one file/line appeared as both `Blocker` and `Task`. The fix narrowly canonicalizes only internal evidence identity from project path, source file, source line, and source text with a leading Markdown checkbox marker removed. Kind-specific result keys, ranking, navigation, public schemas, scanner output, and read-only safety boundaries are unchanged.
...
| Final-review checkbox deduplication RED | `npx vitest run tests/components/search-pure.test.tsx` | Intended regression failure: expected 5 results to equal 4; the other 17 search tests passed. |
| Final-review checkbox deduplication GREEN | `npx vitest run tests/components/search-pure.test.tsx` | 18/18 passed after the narrow evidence-identity fix and collision-boundary controls. |
| Final focused scanner/run-scan gate | `node --test tests/scan-trust.test.mjs tests/run-scan.test.mjs` | 19/19 passed. |
| Final full automated gates | `npm test` | 133/133 Node tests and 105/105 Vitest tests passed. Port 24678 messages appeared while the development server was already using the Vite WebSocket port; no test failed. |
| Final production build | `npm run build` | TypeScript no-emit and Vite build passed. |
| Final strict OpenSpec gate | `openspec validate --all --strict` | Passed for all 13/13 items. |
