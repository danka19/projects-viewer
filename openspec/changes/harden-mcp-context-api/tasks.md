...
- [ ] 1.1 Add regression tests proving a clean setup creates no tracked projects and does not read root `projects.config.json`.
...
- [ ] 2.1 Add a pure helper that maps saved config projects to compact project identity objects.
- [ ] 2.2 Add `GET /api/configured-projects` with safe query handling and no arbitrary path input.
- [ ] 2.3 Add tests for enabled/disabled projects, tags, empty config, and optional scan status joining where implemented.
...
- [ ] 3.1 Add regression tests showing unknown `/api/*` routes return JSON `404`, not the Vite HTML shell.
...
- [ ] 4.1 Update MCP `requestJson()` to reject non-JSON responses even when HTTP status is 200.
- [ ] 4.2 Include status, content type, API path, and a short body preview in MCP errors.
